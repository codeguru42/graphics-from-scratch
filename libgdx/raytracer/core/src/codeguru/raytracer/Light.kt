...
interface Light {
    fun getIntensityAt(p: Point, n: Vector): Float
}

class AmbientLight(val intensity: Float) : Light {
    override fun getIntensityAt(p: Point, n: Vector): Float {
        return intensity
    }
}

class PointLight(val intensity: Float, val position: Point) : Light {
    override fun getIntensityAt(p: Point, n: Vector): Float {
